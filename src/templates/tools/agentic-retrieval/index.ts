...
function createCustomAgenticRetrievalToolLoopAgent({
    ...
    generate: (args: { query: string, onFinish: (output: AgenticRetrievalOutput) => void }) => AsyncGenerator<AgenticRetrievalOutput>;
} {
        ...
        generate: async function* ({
            ...
            while (!finished && currentStep < maxSteps) {
                ...
                const systemPrompt = `
                            ...
                            AVAILABLE TOOLS:
                            ${Object.keys({ ...tools, ...dynamicTools }).map(tool => `
                                <tool_${tool}>
                                    <name>${tool}</name>
                    ...
                    reasoningOutput = await withRetry(async () => {
                        ...
                        return await generateText({
                            ...
                            output: Output.object({
                                schema: z.object({
                                    reasoning: z.string().describe("The reasoning for the next step and why the agent needs to take this step. It MUST start with 'I must call tool XYZ', and MUST include the inputs for that tool."),
                                    finished: z.boolean().describe("Whether the agent has finished meaning no further steps are needed, this should only be true if the agent believes no further tool calls are needed to get the relevant information for the query."),
                                })
                            }),
                            toolChoice: 'none',
                            system: systemPrompt,
                ...
                const exludedContent = toolCalls?.some(toolCall =>
                ...
                for (const chunk of chunks) {
                    ...
                    if (!dynamicTools[getMoreToolName]) {
                        ...
                        const countChunksQuery = await db.from(chunksTable).where(
                        ...
                        if (chunksCount > 1) {
                            dynamicTools[getMoreToolName] = tool({
                                ...
                                inputSchema: z.object({
                                    from_index: z.number().default(1).describe("The index of the chunk to start from."),
                                    to_index: z.number().max(chunksCount).describe("The index of the chunk to end at, max is " + chunksCount),
                                }),
                                execute: async ({ from_index, to_index }) => {
                                    const chunks = await db(chunksTable).select("*").where("source", chunk.item_id).whereBetween("chunk_index", [from_index, to_index]).orderBy("chunk_index", "asc");
                                    return JSON.stringify(chunks.map(resultChunk => ({
                    ...
                    if (exludedContent) {
                        ...
                        dynamicTools[getContentToolName] = tool({
                            ...
                            inputSchema: z.object({
                                reasoning: z.string().describe("The reasoning for why you need to get the content of the page."),
                            }),
                            execute: async ({ reasoning }) => {
                                const { db } = await postgresClient();
                                const chunksTable = getChunksTableName(chunk.context.id);
                                const resultChunks = await db(chunksTable).select("*").where("id", chunk.chunk_id).limit(1);
...
const createAgenticRetrievalAgent = ({
    ...
    generate: (args: { query: string, reranker?: ExuluReranker, onFinish: (output: AgenticRetrievalOutput) => void }) => AsyncGenerator<AgenticRetrievalOutput>;
} => {
...
export const createAgenticRetrievalTool = ({
    ...
    return new ExuluTool({
        ...
        inputSchema: z.object({
            ...
            userInstructions: z.string().optional().describe("Instructions provided by the user to customize the retrieval process."),
        }),
        execute: async function* ({ query, userInstructions, toolVariablesConfig }: { query: string, userInstructions?: string, instructions?: string, [key: string]: any }) {
