...
const TodoWriteTool = new ExuluTool({
  ...
  execute: async (inputs) => {
    ...
    if (!user) {
      throw new Error(
        "No authenticated user available, a user is required for the todo write tool, this likely means the tool was called outside a session like in an MCP or API call instead of as part of an authenticated session.",
      );
    }

    if (!sessionID) {
      throw new Error(
        "Session ID is required for the todo write tool, this likely means the tool was called outside a session like in an MCP or API call instead of as part of a conversation.",
      );
    }
...
export const todoTools = [TodoWriteTool, TodoReadTool];
