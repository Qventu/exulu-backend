...
import { ExuluAgent, ExuluContext, ExuluReranker, ExuluTool, getChunksTableName, getTableName } from "src/exulu/classes";
...
const removeAgentFields = (requestedFields: string[]) => {
    const filtered = requestedFields.filter(
        (field) => !exuluBackendAgentFields.includes(field),
    );
