...
import { IMP_SH_B64 } from "./imp-sh.generated.ts";

export const BOOTSTRAP_CLIENTS_JSON = JSON.stringify(CLIENT_MANIFEST, null, 2);

/**
 * The `imp` helper script shipped inside the bootstrap skill. The agent runs
 * it instead of hand-writing curl: the script reads the API token from
 * `~/.config/imp/skills.json` (so the token never enters the model context)
...
export const BOOTSTRAP_IMP_SH = Buffer.from(IMP_SH_B64, "base64")
  .toString("utf8")
