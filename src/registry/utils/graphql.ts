...
import {
  VectorMethodEnum,
  type VectorMethod,
} from "@EXULU_TYPES/models/vector-methods";
import {
...
const handleRBACUpdate = async (
  ...
  const existingProjectRecords = new Set(
    existingRbacRecords
...
function createMutations(
      ...
      if (
        ...
        (!user.role ||
          (!(table.name.plural === "agents" && user.role.agents === "write") &&
            !(
              table.name.plural === "workflow_templates" &&
              user.role.workflows === "write"
            ) &&
            !(
              table.name.plural === "variables" &&
              user.role.variables === "write"
            ) &&
  ...
  const mutations = {
    ...
    [`${tableNamePlural}CreateOne`]: async (_, args, context, info) => {
      ...
      if (
        table.name.singular === "user" &&
        context.user?.super_admin !== true
      ) {
    ...
    [`${tableNamePlural}UpdateOne`]: async (_, args, context, info) => {
      ...
      await validateCreateOrRemoveSuperAdminPermission(
        tableNamePlural,
        input,
        req,
      );
      ...
      if (!result.id) {
        throw new Error(
          "Something went wrong with the update, no id returned.",
        );
      }
  ...
  if (table.type === "items") {
    if (table.processor) {
      mutations[`${tableNameSingular}ProcessItem`] = async (
        ...
        if (!args.item) {
          throw new Error(
            "Item argument missing, the item argument is required.",
          );
        }
        const { db } = context;
        let query = db
        ...
        if (!item) {
          throw new Error(
            "Item not found, or your user does not have access to it.",
          );
        }
    ...
    mutations[`${tableNameSingular}ExecuteSource`] = async (
      _,
      args,
      context,
      info,
      ...
      if (!args.source) {
        throw new Error(
          "Source argument missing, the source argument is required.",
        );
      }
      ...
      if (!source) {
        throw new Error(
          `Source ${args.source} not found in context ${exists.id}.`,
        );
      }
      ...
      for (const item of result) {
        ...
        if (createdItem.id) {
          ...
          console.log(
            `[EXULU] created item through source update job ${createdItem.id}`,
          );
        }
      }
    ...
    mutations[`${tableNameSingular}GenerateChunks`] = async (
      _,
      args,
      context,
      info,
    ...
    mutations[`${tableNameSingular}DeleteChunks`] = async (
      _,
      args,
      context,
      info,
...
const removeAgentFields = (requestedFields: string[]) => {
  const filtered = requestedFields.filter(
    (field) => !backendAgentFields.includes(field),
  );
...
function createQueries(
  ...
  if (table.type === "items") {
    queries[`${tableNamePlural}VectorSearch`] = async (
      _,
      args,
      context,
      info,
    ...
    queries[`${tableNameSingular}ChunkById`] = async (
      _,
      args,
      context,
      info,
...
export const vectorSearch = async ({
  ...
  if (!Object.values(VectorMethodEnum).includes(method)) {
    throw new Error(
      ...
      Object.values(VectorMethodEnum).join(", "),
    );
  }

  const { id, queryRewriter, embedder, configuration, resultReranker } =
    context;
  ...
  cutoffs = {
    cosineDistance:
      cutoffs?.cosineDistance ||
      context.configuration?.cutoffs?.cosineDistance ||
      0,
    tsvector:
      cutoffs?.tsvector || context.configuration?.cutoffs?.tsvector || 0,
  ...
  switch (method) {
    case "tsvector":
      ...
      chunksQuery
        .select(
          db.raw(`ts_rank(chunks.fts, to_tsquery(?, ?)) as fts_rank`, [
            language,
            orQuery,
          ]),
        )
    ...
    case "hybridSearch":
      ...
      let semanticQuery = db(chunksTable + " as chunks")
        .select([
          ...
          "chunks.source",
          db.raw(
            `row_number() OVER (ORDER BY chunks.embedding <=> ${vectorExpr} ASC) AS rank_ix`,
          ),
        ])
        .leftJoin(mainTable + " as items", "items.id", "chunks.source")
        .whereNotNull("chunks.embedding")
        .whereRaw(`(1 - (chunks.embedding <=> ${vectorExpr})) >= ?`, [
          cutoffs?.cosineDistance || 0,
        ])
        .whereRaw(`(items.archived IS FALSE OR items.archived IS NULL)`)
        .limit(Math.min(matchCount * 2, 500));
      ...
      let hybridQuery = db
        ...
        .select([
          ...
          db.raw('items."updatedAt" as item_updated_at'),
          db.raw('items."createdAt" as item_created_at'),
          db.raw(`ts_rank(chunks.fts, plainto_tsquery(?, ?)) AS fts_rank`, [
            language,
            query,
          ]),
          db.raw(
        ...
        .whereRaw(
          `(chunks.fts IS NULL OR ts_rank(chunks.fts, plainto_tsquery(?, ?)) > ?)`,
          [language, query, cutoffs?.tsvector || 0],
        )
        .whereRaw(
          `(chunks.embedding IS NULL OR (1 - (chunks.embedding <=> ${vectorExpr})) >= ?)`,
          [cutoffs?.cosineDistance || 0],
        )
...
export const contextToTableDefinition = (
  context: ExuluContext,
): ExuluTableDefinition => {
...
export function createSDL(
  ...
  for (const table of tables) {
    ...
    const tableNameSingular = table.name.singular.toLowerCase();
    const tableNameSingularUpperCaseFirst =
      table.name.singular.charAt(0).toUpperCase() +
      table.name.singular.slice(1);
...
`;
    ...
    if (table.RBAC) {
      ...
      resolvers[rbacResolverName].RBAC = async (
        parent: any,
        args: any,
        context: any,
      ) => {
  ...
  resolvers.Query["workflowSchedule"] = async (_, args, context, info) => {
    ...
    const hasAccessToWorkflowTemplate = await checkRecordAccess(
      workflowTemplate,
      "write",
      user,
    );
    ...
    const agentBackend = agents.find(
      (agent) => agent.id === agentInstance.backend,
    );
  ...
  resolvers.Mutation["upsertWorkflowSchedule"] = async (_, args, context, info) => {
    ...
    if (!queue) {
      throw new Error("Queue not found for agent backend: " + agentBackend?.id + " for workflow template: " + workflow_template_id);
    }
  ...
  resolvers.Mutation["runWorkflow"] = async (_, args, context, info) => {
    ...
    if (queue) {
      ...
      console.log(
        "[EXULU] running a workflow directly without queue.",
        jobData.label,
      );
      ...
      try {
        ...
        console.error(
          `[EXULU] error running workflow ${workflow_template_id}.`,
          jobData.label,
          {
            error: error instanceof Error ? error.message : String(error),
          },
  ...
  resolvers.Mutation["runEval"] = async (_, args, context, info) => {
    ...
    // Fetch the eval run
    const evalRun: EvalRun = await db
      .from("eval_runs")
    ...
    if (args.test_case_ids) {
      testCaseIds = testCaseIds.filter((testCase) =>
        args.test_case_ids.includes(testCase),
      );
    }
  ...
  resolvers.Query["jobs"] = async (_, args, context, info) => {
    ...
    return {
      ...
      pageInfo: {
        ...
        hasNextPage:
          args.page && args.page < Math.ceil(count / (args.limit || 100))
            ? true
            : false,
      },
    };
        ...
        if (context.processor) {
          processor = await new Promise(async (resolve, reject) => {
            ...
            resolve({
              ...
              generateEmbeddings:
                context.processor?.config?.generateEmbeddings || false,
            });
          });
        }
  ...
  resolvers.Query["contextById"] = async (_, args, context, info) => {
    let data: ExuluContext | undefined = contexts.find(
      (context) => context.id === args.id,
    );
    ...
    if (data.processor) {
      processor = await new Promise(async (resolve, reject) => {
        ...
        resolve({
          ...
          timeoutInSeconds: queue?.timeoutInSeconds || 600,
          generateEmbeddings:
            data.processor?.config?.generateEmbeddings || false,
        });
      });
    }
  ...
  resolvers.Query["tools"] = async (_, args, context, info) => {
    ...
    let agentTools = await Promise.all(
      instances.map(async (instance: Agent) => {
        const backend: ExuluAgent | undefined = agents.find(
          (a) => a.id === instance.backend,
        );
        if (!backend) {
          return null;
        }
...
const validateCreateOrRemoveSuperAdminPermission = async (
  ...
  if (tableNamePlural === "users" && input.super_admin !== undefined) {
    ...
    if (!authResult.user.super_admin) {
      throw new Error(
        "Only super administrators can modify super_admin status",
      );
    }
  }
};

async function getJobsByQueueName(
  queueName: string,
  statusses?: JobState[],
  page?: number,
  limit?: number,
