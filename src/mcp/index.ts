...
export class ExuluMCP {
  ...
  private configure = async ({
    ...
    if (!server.tools[getListOfPromptTemplatesName]) {
      server.mcp.registerTool(
        getListOfPromptTemplatesName,
        {
          title: "Get List of Prompt Templates",
          description:
            "Retrieves a list of prompt templates available for this agent. Returns the name, description, and ID of each template.",
          inputSchema: {
            inputs: z.object({}),
          },
        },
    ...
    if (!server.tools[getPromptTemplateDetailsName]) {
      server.mcp.registerTool(
        getPromptTemplateDetailsName,
        {
          title: "Get Prompt Template Details",
          description:
            "Retrieves the full details of a specific prompt template by ID, including the actual template content with variables.",
          inputSchema: {
            inputs: z.object({
        ...
        async ({ inputs }, args): Promise<any> => {
          ...
          const isAssignedToAgent = await db
            .from("prompt_library")
            .select("id")
            .where({ id: inputs.id })
            .whereRaw("assigned_agents @> ?::jsonb", [JSON.stringify(agent.id)])
