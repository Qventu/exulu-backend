...
- **Zone exclusivity is hide, not disable.** Once one zone has content the other is removed from the layout entirely (single-column card) rather than greyed out; a hint in the active zone explains how to switch back. The files list gained a "Remove all" button.
- **Run-phase clarity (UAT bug).** Progress divides by rows actually queued, so skipped-invalid rows no longer freeze the bar short of 100%. The done state shows a green check when nothing failed plus an "N rows skipped" note; rows that never ran get a "Skipped" badge; and a re-import button appears for rows fixed after the run.
