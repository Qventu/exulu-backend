...
- Case 1 passes **both** `preselected` (project gids — hard scope) and `projectScope` (identity, instructions, profile defaults, items). The §3.3 union semantics are structural no-ops under preselection: `search.ts:76-82` derives pins *from* preselection when present, and routing's always-main append duplicates the preselected contexts. One code path, two cases.
...
- **Profiles:** a project-added context without a stored `knowledge_bases` profile uses `kbProfileDefaults` (§7.3), else kind preset `documents`.
- **Config switch:** new flat option `project_search` (boolean, default `true`, option 13) — when false, convert passes no `projectScope` and performs no Case-1 injection for this agent.
...
| `custom_instructions` becomes live | Previously stored-but-dead; now influences retrieval routing. |
