...
semantic AS (
  ...
  WHERE c.embedding IS NOT NULL
    AND (1 - (c.embedding <=> ${VECTOR_EXPR})) >= 0.2
    AND (i.archived IS FALSE OR i.archived IS NULL)
  LIMIT 100
)
SELECT
  ...
  i.name as item_name,
  i.id as item_id,
  (SELECT COUNT(*) FROM {context_id}_chunks WHERE source = i.id) as total_chunks_in_item,
...
- **CRITICAL:** The query includes `total_chunks_in_item` - use this to decide if you need the file + grep workflow (Step 7.5)
- **Increased LIMIT to 30** (from 10) to cast a wider net and reduce follow-up queries
- **Lowered similarity threshold to 0.2** (from 0.3) to capture more potentially relevant content
...
Load environment variables ONCE at the start of your workflow, not for every command:
...
For all subsequent commands, simply use the already-loaded variables:
```bash
PGPASSWORD=$POSTGRES_DB_PASSWORD psql -h $POSTGRES_DB_HOST -p $POSTGRES_DB_PORT ...
```

Do NOT reload `.env` for each command - this wastes time.
...
### Step 7.5: Smart Document Navigation Strategy (CRITICAL)

After your initial search, you need to decide how to retrieve additional content from relevant documents.

**⚠️ CRITICAL: Avoid Context Window Overflow**

**Step A: Count chunks in relevant documents**

From your initial search results, identify the most relevant document(s) by their `item_id`. Check how many chunks each has:
...
WHERE source = '{item_id_from_initial_search}';
"
```

**Step B: Choose strategy based on document size**

**Strategy 1: Small Documents (≤ 10 chunks) - Load Directly**

Safe to load all chunks into context:
...
**Strategy 2: Large Documents (> 10 chunks) - Save to File + Grep (REQUIRED)**

**DO NOT load directly** - this will overflow your context window.

**Workflow:**

```bash
# Step 1: Save full document to temporary file (does NOT consume context tokens)
PGPASSWORD=$POSTGRES_DB_PASSWORD psql -h $POSTGRES_DB_HOST -p $POSTGRES_DB_PORT \
  -U $POSTGRES_DB_USER -d $POSTGRES_DB_NAME -c "
SELECT
  '### CHUNK ' || chunk_index || ' ###' || E'\n' as header,
...
WHERE source = '{item_id}'
ORDER BY chunk_index ASC;
" > /tmp/document_full.txt

echo "✓ Document saved: $(wc -l < /tmp/document_full.txt) lines"

# Step 2: Grep for specific information (fast, no context token cost)
echo "=== Searching for relevant sections ==="

# Use multiple grep passes to narrow down
grep -i "search_term_1" /tmp/document_full.txt > /tmp/pass1.txt
grep -i "search_term_2" /tmp/pass1.txt > /tmp/pass2.txt
grep -B 2 -A 10 -i "specific_keyword" /tmp/pass2.txt

# Step 3: Identify which chunks contain the relevant data
grep "### CHUNK" /tmp/pass2.txt | sort -u

# Output example: CHUNK 5, CHUNK 12, CHUNK 23

# Step 4: Load ONLY those specific chunks (minimal context usage)
PGPASSWORD=$POSTGRES_DB_PASSWORD psql -h $POSTGRES_DB_HOST -p $POSTGRES_DB_PORT \
  -U $POSTGRES_DB_USER -d $POSTGRES_DB_NAME -c "
SELECT chunk_index, content
FROM {context_id}_chunks
WHERE source = '{item_id}'
  AND chunk_index IN (5, 12, 23)
ORDER BY chunk_index ASC;
"
```

**Token Usage Comparison:**
- ❌ Loading 85 chunks directly: ~102,000 tokens (context overflow)
- ✅ File + grep + load 3 chunks: ~3,600 tokens (96% savings)

**Multi-Pass Grep Strategy for Complex Queries:**

```bash
# Pass 1: Broad topic filter
grep -i "elevator\|lift\|aufzug" /tmp/document_full.txt > /tmp/pass1.txt
echo "Pass 1: $(wc -l < /tmp/pass1.txt) lines"

# Pass 2: Geographic/entity filter
grep -i "germany\|deutschland\|specific_entity" /tmp/pass1.txt > /tmp/pass2.txt
echo "Pass 2: $(wc -l < /tmp/pass2.txt) lines"

# Pass 3: Data type filter
grep -i "statistics\|total\|anzahl\|existing" /tmp/pass2.txt > /tmp/pass3.txt
echo "Pass 3: $(wc -l < /tmp/pass3.txt) lines"

# Now identify relevant chunks
grep "### CHUNK" /tmp/pass3.txt | grep -o "CHUNK [0-9]*" | sort -u
```

**When to use file + grep:**
- Document has > 10 chunks
- You're searching for specific data points (statistics, dates, names)
- You need to scan the entire document but only extract specific sections
- User asks questions that require finding needles in haystacks
...
### Respect Context Window Limits (CRITICAL)

**Be aware of context token consumption to avoid overflow:**

**Typical chunk sizes:**
- Small chunks: 300-800 tokens
- Medium chunks: 800-1,500 tokens
- Large chunks: 1,500-3,000 tokens
- Average: ~1,200 tokens per chunk

**Context window budget:**
- Total available: ~200,000 tokens
- Safe working limit: ~150,000 tokens (leave room for output)
- Critical threshold: 100,000 tokens (50% usage)

**Safe loading limits:**
- **< 10 chunks:** Load directly (~12,000 tokens) ✅
- **10-50 chunks:** Use file + grep, load max 10 specific chunks (~12,000 tokens) ⚠️
- **> 50 chunks:** ALWAYS use file + grep, load max 5-10 chunks (~6,000-12,000 tokens) ❌ Never load all

**Quick chunk count check:**
```bash
PGPASSWORD=$POSTGRES_DB_PASSWORD psql -h $POSTGRES_DB_HOST -p $POSTGRES_DB_PORT \
  -U $POSTGRES_DB_USER -d $POSTGRES_DB_NAME -t -c "
SELECT COUNT(*) FROM {context_id}_chunks WHERE source = '{item_id}';
"
```

**If count > 10: MANDATORY to use file + grep workflow from Step 7.5**

**Example token calculation:**
- 85 chunks × 1,200 tokens/chunk = 102,000 tokens (51% of context window) ❌
- 200 chunks × 1,200 tokens/chunk = 240,000 tokens (OVERFLOW - will fail) ❌
- 3 targeted chunks × 1,200 tokens/chunk = 3,600 tokens (2% of context window) ✅

### Always Load Environment Once

Load `.env` ONCE at the beginning of your workflow (see Step 7), not for every command.
...
Then for all subsequent commands, just use the variables:
```bash
PGPASSWORD=$POSTGRES_DB_PASSWORD psql -h $POSTGRES_DB_HOST ...
```
...
- **Don't load large documents directly** - ALWAYS check chunk count first, use file + grep for > 10 chunks (see Step 7.5)
- **Don't overflow context window** - Loading 50+ chunks will fail; respect token budgets
- **Don't reload .env repeatedly** - Load once at the start, reuse for all commands
- **Don't use small LIMIT values** - Use LIMIT 30 (not 10) to reduce follow-up queries
