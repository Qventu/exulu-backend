...
| `poppler-utils` | PDF rendering (`pdfinfo`, `pdftoppm`) and text extraction (`pdftotext`) for the document processor. `pdftotext` is probed at startup as a required binary — the backend refuses to start when `requireSystemDependencies` is enabled; otherwise it logs a warning and document extraction degrades. |
