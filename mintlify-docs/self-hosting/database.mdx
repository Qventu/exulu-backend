...
<Warning>
**Rotating `NEXTAUTH_SECRET` breaks stored tool credentials.** Because each blob is encrypted with the current secret, changing `NEXTAUTH_SECRET` makes every previously stored credential undecryptable. Users will need to re-enter their credentials for every connected tool after a secret rotation.
</Warning>
