...
|---|---|---|
| `POST /webhooks/routine/{secret}` | Email triggers (inbound email) | Each routine trigger has its own capability-URL secret. The backend parses raw RFC 822 MIME, Mailgun-style multipart, or a JSON body. Optional per-trigger HMAC signing (`X-Exulu-Signature`). The email is persisted before the 2xx ACK is returned so that a restart mid-processing does not silently lose the message. |
...
**Exposure guidance.** Both paths must be reachable on the public URL you set in `BACKEND` (used for routine webhook URLs and for Recall webhook subscription).
