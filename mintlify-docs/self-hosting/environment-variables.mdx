...
| `BACKEND` | Yes | Server, Frontend | Public URL of the backend API — used in OAuth callbacks, Recall webhooks, per-trigger inbound-email webhook URLs (`/webhooks/routine/{secret}`), and tool-credential submit URLs. Must be reachable from the browser, from Recall's servers, and from any inbound-email delivery source. | `https://api.imp.example.com` |
...
These variables control rate limiting for logged-in users with the `external` role (self-registered users invited to chat with specific agents). Both are optional; the backend uses the defaults listed when the variables are absent. Unlike guest rate limiting, this is keyed by user ID rather than IP, since external users are authenticated.

| Variable | Required? | Component | Default | Purpose | Example |
|---|---|---|---|---|---|
| `EXULU_EXTERNAL_RATE_PER_MINUTE` | No | Server | `30` | Maximum messages a single external user may send within any rolling one-minute window. Exceeding this returns HTTP 429. | `20` |
| `EXULU_EXTERNAL_RATE_PER_HOUR` | No | Server | `300` | Maximum messages a single external user may send within any rolling one-hour window. Exceeding this returns HTTP 429. | `500` |
