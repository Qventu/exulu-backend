...
| `guest_access` | When `true`, the agent is accessible without authentication via a public guest link. |
| `guest_auth_mode` | Controls how unauthenticated guest visitors authenticate: `"none"` (open), `"password"` (shared password required), or `"sso"` (IdP-backed). |
