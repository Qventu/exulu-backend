...
<Note>
`groupBy` must name a defined, non-hidden field (or one of the always-allowed synthetic fields `id`, `createdAt`, `updatedAt`). Passing a hidden field name or an unknown column name causes the resolver to throw: `Cannot group by "{field}".` — no rows are returned.
</Note>
...
<Note>
`sort.field` must name a defined, non-hidden field (or one of `id`, `createdAt`, `updatedAt`). Passing a hidden field name or an unknown column name throws: `Cannot sort by "{field}".`
</Note>
...
On RBAC-enabled tables, `{plural}CreateOne` accepts an explicit `rights_mode` in the input. Allowed values are `private`, `users`, `roles`, `teams`, and `public`. If the field is omitted the record is created as `private`; if an unrecognised value is supplied the mutation throws `Invalid rights_mode "..." — expected one of: private, users, roles, teams, public` and no row is inserted. `{plural}CopyOneById` always creates the copy as `private` regardless of the source record's `rights_mode`.
