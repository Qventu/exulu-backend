...
| `GET` | `/skills/registry/:name` | Required | Metadata for a single skill by name. Returns `404` when the skill does not exist; `403` when the caller lacks read access. |
