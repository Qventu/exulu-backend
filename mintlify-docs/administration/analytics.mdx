...
When the total across all tagged entities (users, teams, agents) is meaningfully lower than the raw gateway total, the Spend, Tokens, and Requests cards show a second caption line — for example, **+ $0.42 unattributed** or **+ 1,200 unattributed**. This indicates usage not attributed to any user, team, or agent tag — typically direct gateway traffic that bypassed the IMP tag layer. If the gap is 1% or less it is suppressed.
