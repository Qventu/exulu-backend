...
<Note>
The `/configuration` route has a sibling page: [Email intake](/administration/email-intake) (`/configuration/email`), where you connect IMP to a Mailgun inbound domain for email-triggered routines. The two pages share the same path prefix but serve different administration tasks.
</Note>
