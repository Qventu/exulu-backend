...
The **Theme** page (`/configuration`) — labelled "Theme" in the sidebar — is the white-label configuration surface for IMP.

<Note>
The `/configuration` route has a sibling page: [Email intake](/administration/email-intake) (`/configuration/email`), which documents how routine email triggers receive inbound mail via per-trigger secret webhook URLs. The two pages share the same path prefix but serve different administration tasks.
</Note>

It lets you override the platform's CSS variables (colors, typography, shapes) for both light and dark modes, see a live preview before publishing, and roll out changes to all users at once.
