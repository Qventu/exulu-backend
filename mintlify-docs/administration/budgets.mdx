...
- **Show budget status to users** — when enabled, users see a **Budget `{percent}`%** chip in the application's top bar. This toggle also controls whether the **Usage** section appears in a user's Settings page and whether the top-bar chip's **Details** link to personal usage is active. When the toggle is off, none of these surfaces render.
