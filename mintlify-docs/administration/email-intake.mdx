...
Click **Save settings** to apply changes. The webhook URL section becomes active after the first save.

**Webhook URL** — a read-only copy field showing the URL to paste into the Mailgun route. The URL is derived from the platform's public base URL (`{BACKEND}/webhooks/email/mime`). If the environment variable is not set the field shows a pending message; contact your infrastructure team.
