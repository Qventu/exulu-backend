...
Click **New item** in the top-right of the context workspace. The **New item** dialog asks for:
...
Use the [Bulk import](/building/knowledge/bulk-import) flow to create many items at once from a CSV or spreadsheet file. Bulk import maps columns to item fields and queues the pipeline automatically after upload.

**3. Agent-created items**

When the **Knowledge base editor** tool is enabled for an agent, the agent can create and update items during a chat session. See [Tools and skills](/building/agents/tools-and-skills) for how to enable the tool and configure which contexts the agent can write to.
...
<Columns cols={2}>
  ...
  <Card title="Bulk import" icon="upload" href="/building/knowledge/bulk-import">
    Create many items at once from a CSV or spreadsheet file.
  </Card>
  <Card title="Tools and skills" icon="wrench" href="/building/agents/tools-and-skills">
    Enable the Knowledge base editor so agents can write items during chat.
  </Card>
</Columns>
