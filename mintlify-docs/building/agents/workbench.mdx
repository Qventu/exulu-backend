...
| **File sandbox** | Toggle to give the agent a file sandbox with `readFile`, `writeFile`, and `bash` tools. Required for skills and file generation. Off by default. The document tools (`parse_document`, `view_document_page`) are available in all chat sessions with file storage configured, regardless of this toggle. See [Attachments and session files](/user-guide/chat/attachments-and-session-files) for how users upload and reference files in chat. |
...
When the agent firewall feature is enabled on your deployment, an additional Safety section appears in the editor.
