...
<Note>
**Breaking (July 2026):** the `oauth` constructor option was renamed `authentication` and requires an explicit `authType: "oauth"` tag. See [ExuluTool — configuration](/developers/core/exulu-tool/configuration) for the full reference.
</Note>

When a tool needs to act on behalf of the signed-in user — reading their calendar, sending email from their account, or accessing a SaaS workspace — declare an `authentication` config with `authType: "oauth"` on the tool. IMP handles the authorization-code flow including PKCE, token storage per user, and automatic retry after the user consents. Your `execute` function receives the access token as `inputs.oauth.accessToken`.
...
When `authentication` is declared on a tool:
...
5. **Authorization URL fallback** — When no valid token exists for the calling user, IMP short-circuits `execute` and returns an authorization URL. The agent surfaces this URL to the user ("Click here to connect your calendar"). After the user consents, IMP retries the original tool call with the fresh token. Internally the short-circuit result carries `oauth: { authorizationUrl }`, which the chat interface renders as a **Connect** button.

Your code only needs to declare the authentication config and use `inputs.oauth.accessToken`.
...
export const calendarEventsTool = new ExuluTool({
  ...
  execute: async ({ days = 7, calendar_id, oauth, toolVariablesConfig }) => {
    // oauth and toolVariablesConfig are both injected by the framework onto the
    // FIRST argument. If no token exists, IMP never reaches execute — it returns
    // the auth URL instead.
    const apiBase = toolVariablesConfig.calendar_api_base_url
