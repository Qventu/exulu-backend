...
export const orderLookupTool = new ExuluTool({
  ...
  execute: async ({ orderId, includeLineItems = false, toolVariablesConfig }) => {
    const apiUrl = toolVariablesConfig.orders_api_url ?? "https://orders.internal/api/v2";
    const apiKey = toolVariablesConfig.orders_api_key;
...
- **Reading those params** — they arrive as `toolVariablesConfig` on the **first** argument, keyed by param name, already decrypted. They are not on the second (`options`) argument, which carries only AI-SDK context. See [Reading config values at runtime](/developers/core/exulu-tool/configuration#reading-config-values-at-runtime). If an administrator has not configured the tool, `toolVariablesConfig` is `{}` — the `??` fallback above is deliberate for a URL, but never fall back to a default credential or tenant.
...
export const batchReportTool = new ExuluTool({
  ...
  execute: async function* ({ accountIds, period, toolVariablesConfig }) {
    const apiUrl = toolVariablesConfig.reporting_api_url ?? "https://reporting.internal/api";
