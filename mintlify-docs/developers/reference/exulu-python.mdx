...
Verifies that the virtual environment exists, that the Python executable works, and (when `checkPackages` is `true`) that the critical packages `pypdf` and `transformers` can be imported.

<ParamField path="checkPackages" type="boolean" default="true">
  Whether to import-check `pypdf` and `transformers` in addition to checking the executable.
</ParamField>
...
| Disk | ~2 GB for `transformers` models on first run |
