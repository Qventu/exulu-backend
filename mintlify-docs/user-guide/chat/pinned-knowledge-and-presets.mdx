...
If you own a preset or have been granted write access to it, each preset row in the **Add knowledge** dialog shows two additional controls:

- **Edit (pencil icon)** — opens the preset in edit mode so you can rename it, update the description, or change the pinned selection.
- **Delete** — shows an inline confirmation strip on the same row before the preset is removed. Click **Delete** once to reveal the confirmation, then click **Delete** again to confirm, or **Cancel** to dismiss it.

These controls are only visible to the preset's creator, administrators, and any users or roles explicitly granted write access. Viewers and users with read-only access see only the **Apply** button.
