...
If a meeting job gets stuck in Processing — for example, because of a service restart while the bot was active — IMP runs an automatic reconcile sweep every minute that picks up the stalled job and drives it forward. This means you do not need to take any action; the job resumes on its own.

<Note>
The reconcile sweep has a 24-hour limit measured from the meeting start. If a job cannot be recovered within that window it is marked as failed. This prevents jobs that genuinely cannot complete from staying in Processing indefinitely.
</Note>
